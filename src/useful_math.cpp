...
//    Mres=W.inverse()*M.transpose()* pseudoInv_SVD(Mres);
    ...
    Eigen::CompleteOrthogonalDecomposition<Eigen::MatrixXd> cqr(Mres);
    Mres=W.inverse()*M.transpose()* cqr.pseudoInverse();
    return Mres;
}
...
Eigen::MatrixXd dyn_pseudoInv(const Eigen::MatrixXd &M, const Eigen::MatrixXd &dyn_M, bool isMinv) {
    ...
    Eigen::CompleteOrthogonalDecomposition<Eigen::MatrixXd> cqr(temp);
    Eigen::MatrixXd res = Minv * M.transpose() * cqr.pseudoInverse();
